...
namespace NautilusFREE
{
    public class MoggFile
    {
        ...
        //private static byte[] C3_PRIVATE_KEY_C = { 0x2D, 0x68, 0x64, 0x6C, 0x94, 0x73, 0x17, 0x47, 0x97, 0x3D, 0x64, 0xDF, 0x89, 0x17, 0x63, 0x8B };
        private static byte[] C3_PRIVATE_KEY_D = { 0xC0, 0x87, 0x69, 0x00, 0xE2, 0x7C, 0x73, 0xEB, 0xCC, 0xD4, 0x21, 0x3D, 0x70, 0x2A, 0x4F, 0xED };
        ...
        {
            //enable for debugging only not for release
            return;
            var path = "nautilusFREE.log";            
        ...
        public bool Decrypt(byte[] mData, bool includeHeader = true, bool includeKey = true, DecryptMode mode = DecryptMode.ToFile, string output = "")
        {
            Log("");
            Log("Starting decryption routine");
            CryptVersion version;
            byte[] STORED_PRIVATE_KEY;
            byte[] DERIVED_PRIVATE_KEY;
            using (var br = new BinaryReader(new MemoryStream(mData)))
            {
                version = (CryptVersion)br.ReadInt32();
            }            
            if (!EncryptionIsSupported(version))
            {
                Log("Encryption type is not supported: " + version);
                return false;
            }
            if (!LoadFromFile(mData))
            {
                Log("Failed to LoadFromFile");
                return false;
            }
            if (PUBLIC_KEY.Length != 0x10 && PUBLIC_KEY.Length != 0x48)
            {
                Log("Public Key Length is invalid: " + PUBLIC_KEY.Length);
                return false;
            }
            ...
            switch (_version)
            {
                case CryptVersion.x0B:
                    DERIVED_PRIVATE_KEY = HMX_PRIVATE_KEY_0B;
                    STORED_PRIVATE_KEY = HMX_PRIVATE_KEY_0B;
                    Log("Using HMX_PRIVATE_KEY_0B");
                    break;
                case CryptVersion.x0C:
                case CryptVersion.x0D:
                    DERIVED_PRIVATE_KEY = MoggCrypt.GenPrivateKey(ref HMX_PRIVATE_KEY_0C, ref mData, false);
                    STORED_PRIVATE_KEY = HMX_PRIVATE_KEY_0C;
                    Log("Using HMX_PRIVATE_KEY_0C");
                    break;
                case CryptVersion.x0E:
                    DERIVED_PRIVATE_KEY = MoggCrypt.GenPrivateKey(ref HMX_PRIVATE_KEY_0E, ref mData, true);
                    STORED_PRIVATE_KEY = HMX_PRIVATE_KEY_0E;
                    Log("Using HMX_PRIVATE_KEY_0E");
                    break;
                case CryptVersion.x0F:
                    DERIVED_PRIVATE_KEY = MoggCrypt.GenPrivateKey(ref HMX_PRIVATE_KEY_0F, ref mData, true);
                    STORED_PRIVATE_KEY = HMX_PRIVATE_KEY_0F;
                    Log("Using HMX_PRIVATE_KEY_0F");
                    break;
                case CryptVersion.x10:
                    DERIVED_PRIVATE_KEY = MoggCrypt.GenPrivateKey(ref HMX_PRIVATE_KEY_10, ref mData, true);
                    STORED_PRIVATE_KEY = HMX_PRIVATE_KEY_10;
                    Log("Using HMX_PRIVATE_KEY_10");
                    break;
                default:
                    return false;
            }            
            Log("Details:");
            Log("Encryption Type: " + version);
            Log("Stored Private Key: " + BitConverter.ToString(STORED_PRIVATE_KEY).Replace("-", ""));
            Log("Derived Private Key: " + BitConverter.ToString(DERIVED_PRIVATE_KEY).Replace("-", ""));
            Log("Public Key: " + BitConverter.ToString(PUBLIC_KEY).Replace("-", ""));
            var decrypted = MoggCrypt.CryptStream(new MemoryStream(OggData), DERIVED_PRIVATE_KEY, PUBLIC_KEY);
            if (!decrypted)
            {
                Log("Failed at MoggCrypt.CryptStream step");
                return false;
            }
            try
            {
                ...
                {
                    Log("Header is not OggS, stopping");
                    return false; //decryption failed
                }
                if (mode == DecryptMode.ToMemory)
                {
                    Log("Decrypted to memory successfully");
                    return true;
                }
            ...
            if (success)
            {
                Log("Decrypted to file successfully");                
                return true;
            }
            else
            {
                Log("Failed to decrypt to file, reason unknown");
                return false;
            }
        }
        ...
        public bool Encrypt(byte[] mData, string mOut)
        {
            Log("");
            Log("Starting encryption routine");
            CryptVersion version;
            using (var br = new BinaryReader(new MemoryStream(mData)))
            {
                version = (CryptVersion)br.ReadInt32();
            ...
            {
                Log("File is already encrypted: " + version);
                return false;
            }
            if (!LoadFromFile(mData))
            {
            ...
            PUBLIC_KEY = MoggCrypt.C3_PUBLIC_KEY_D;//HMX_PUBLIC_KEY_OMG;
            //var DERIVED_PRIVATE_KEY = MoggCrypt.GenPrivateKey(ref C3_PRIVATE_KEY_D, ref mData, false);
            var DERIVED_PRIVATE_KEY = C3_PRIVATE_KEY_D;
            Log("Stored Private Key: " + BitConverter.ToString(C3_PRIVATE_KEY_D).Replace("-", ""));
            Log("Derived Private Key: " + BitConverter.ToString(DERIVED_PRIVATE_KEY).Replace("-", ""));
            Log("Public Key: " + BitConverter.ToString(PUBLIC_KEY).Replace("-", ""));
            FixHMXAHeader(true);
            var encrypted = MoggCrypt.CryptStream(new MemoryStream(OggData), DERIVED_PRIVATE_KEY, PUBLIC_KEY);
            if (!encrypted)
            {
                Log("Failed to encrypt at MoggCrypt.CryptStream");
                return false;
            }
            SaveToFile(mOut, true, true, true);
            var success = File.Exists(mOut);
            if (success)
            {
                Tools.PatchMoggForPS3Use(mOut);
                Log("Encrypted file successfully");
                return true;
            }
            else
            {
                Log("Failed to encrypt file, reason unknown");
                return false;
            }            
        }               
        ...
        private static bool EncryptionIsSupported(CryptVersion version)
        {
            switch (version)
            {
                ...
                //case CryptVersion.xF0: - already decrypted YARG file, not valid for decrypting
                case CryptVersion.x0B:
                case CryptVersion.x0C:
                case CryptVersion.x0D:
                case CryptVersion.x0E:
    ...
    public class MoggCrypt
    {
        ...
        public static readonly byte[] C3_PUBLIC_KEY_D =
        {
        ...
        public static readonly byte[] NEMO_PUBLIC_KEY =        
        {
            0x00, 0x00, 0x65, 0x6E, 0x63, 0x72, 0x79, 0x70, 0x74, 0x65, 0x64, 0x00, 0x62, 0x79, 0x00, 0x6E,
            0x61, 0x75, 0x74, 0x69, 0x6C, 0x75, 0x73, 0x00, 0x6D, 0x61, 0x64, 0x65, 0x00, 0x62, 0x79, 0x00,
            0x6E, 0x65, 0x6D, 0x6F, 0x00, 0x74, 0x68, 0x78, 0x00, 0x74, 0x6F, 0x00, 0x6C, 0x6F, 0x63, 0x61,
            0x6C, 0x68, 0x00, 0x6D, 0x69, 0x6C, 0x6F, 0x68, 0x61, 0x78, 0x00, 0x61, 0x6E, 0x64, 0x00, 0x6D,
            0x61, 0x78, 0x74, 0x6F, 0x6E, 0x00, 0x00, 0x00
        };
        public static readonly byte[] HMX_PUBLIC_KEY_OMG =
        {
            0x00, 0x00, 0x00, 0x00, 0x53, 0x82, 0x0C, 0x05, 0xA2, 0x3C, 0xBD, 0x30, 0x1A, 0x1C, 0xA2, 0x14,
            0x36, 0xEE, 0xC5, 0x70, 0x00, 0x00, 0x00, 0x00, 0x85, 0x8A, 0x14, 0x68, 0x00, 0x00, 0x00, 0x00,
            0xBD, 0x82, 0x42, 0x27, 0x64, 0xD2, 0xAC, 0xFD, 0xA9, 0xE9, 0x06, 0x1F, 0xEA, 0x22, 0x49, 0xE4,
            0x28, 0x26, 0xFF, 0xA5, 0x01, 0x1E, 0x88, 0xF0, 0x73, 0xA9, 0xF1, 0x78, 0x86, 0x23, 0x98, 0x15,
        ...
        private void GenerateCipherOut()
        {
            ...
            if (!cipherOutLogged)
            {
                ...
                moggFile.Log("CipherIn: " + BitConverter.ToString(cipherIn).Replace("-", ""));
                moggFile.Log("CipherOut: " + BitConverter.ToString(cipherOut).Replace("-", ""));
                cipherOutLogged = true;
            }

        }
        private bool cipherOutLogged = false;
    ...
    public enum CryptVersion
    {
        ...
        xF0 = 0xF0, //No encryption YARG file
